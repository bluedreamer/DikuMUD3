...
static void dump_command_history(void)
{
    ...
    for (j = 0, i = command_history_pos; j < MAX_DEBUG_HISTORY; j++)
    {
        slog(LOG_ALL,
             0,
             "CMD %s@%s [%s]",
             command_history_data[i].pcname[0] == '\0' ? command_history_data[i].pcname : command_history_data[i].fi->getName(),
             command_history_data[i].pcname[0] == '\0' ? "" : command_history_data[i].fi->getZone()->getName(),
